...
package Rangee is
   ...
   -- retourne le chiffre place a l'indice i de la rangee r necessite 0 < i
   -- <= taille(r) leve l'exception TRANCHE_INVALIDE si i < 1 ou i > taille(r)
   ...
   -- retourne le chiffre de droite de coordonnée c de la Rangee e necessite
   -- 0 < i <= taille(e) leve l'exception TRANCHE_INVALIDE si i < 1 ou i >
   -- taille(e)
   function chiffreDeDroite
     (E : in Type_Rangee; I : in Integer) return Type_Chiffre;

   -- retourne le chiffre de gauche de coordonnée c de la Rangee e necessite
   -- 0 < i <= taille(e) leve l'exception TRANCHE_INVALIDE si i < 1 ou i >
   -- taille(e)
   function chiffreDeGauche
     (E : in Type_Rangee; I : in Integer) return Type_Chiffre;

   -- retourne VRAI si les chiffre de droite de coordonnée c de la Rangee e
   -- sont égaux et FAUX sinon necessite 0 < i <= taille(e) leve l'exception
   -- TRANCHE_INVALIDE si i < 1 ou i > taille(e)
   function SontDeuxChiffresDeDroiteEgaux
     (E : in Type_Rangee; I : in Integer) return Boolean;

   -- retourne VRAI si les chiffre de gauche de coordonnée c de la Rangee e
   -- sont égaux et FAUX sinon necessite 0 < i <= taille(e) leve l'exception
   -- TRANCHE_INVALIDE si i < 1 ou i > taille(e)
   function SontDeuxChiffresDeGaucheEgaux
     (E : in Type_Rangee; I : in Integer) return Boolean;

   -- remplit la case i de la rangee r avec le chiffre c necessite 0 < i <=
   -- taille(r) leve l'exception TRANCHE_INVALIDE si i < 1 ou i > taille(r)
   function AjouterChiffre
     (R : in Type_Rangee; I : in Integer; C : in Type_Chiffre)
      return Type_Rangee;

   -- positionne la case i de la rangee r avec le chiffre INCONNU necessite
   -- 0 < i <= taille(r) leve l'exception TRANCHE_INVALIDE si i < 1 ou i >
   -- taille(r)
   function RetirerChiffre
     (R : in Type_Rangee; I : in Integer) return Type_Rangee;

   -- Analyse la rangee r avec les règles du takuzu (deux fois le meme
   -- chiffre d'affilée au plus et autant de UN que de ZERO). Renvoie True
   -- si la ligne comporte une erreur et False sinon. Renvoie False si la ligne
   -- n'est pas complète et n'a pas d'erreurs.
   function ComporteErreur (R : in Type_Rangee) return Boolean;
